...
preserve-always list in ground rule 3.
...
6. **No em dashes (U+2014) anywhere in CV or cover-letter copy.** Use commas,
   colons or a full stop instead. En dashes (U+2013) stay: they are correct in
   date ranges (`Jan 2023 – Dec 2025`), numeric ranges (`4–5 levels`) and the
   bullet glyph. Check with `grep -n $'\\u2014'` before every render.
